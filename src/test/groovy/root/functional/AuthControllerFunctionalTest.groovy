...
class AuthControllerFunctionalTest extends BaseFunctionalTest {
    ...
    private static final USER_REGISTRATION_REQUEST_BODY = Resources.getResource('json/user-registration-request.json').text
            .replace('$EMAIL', EMAIL)
            .replace('$PASSWORD', PASSWORD_1)
    private static final USER_ACTIVATION_REQUEST_BODY = Resources.getResource('json/user-activation-request.json').text
            .replace('$EMAIL', EMAIL)
            .replace('$VERIFICATION_CODE', VERIFICATION_CODE)
    private static final SIGN_IN_REQUEST_BODY = Resources.getResource('json/sign-in-request.json').text
            .replace('$EMAIL', EMAIL)
            .replace('$PASSWORD', PASSWORD_1)
    private static final RESET_PASSWORD_REQUEST_BODY = Resources.getResource('json/reset-password-request.json').text
            .replace('$EMAIL', EMAIL)
